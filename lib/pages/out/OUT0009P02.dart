...
class OUT0009P02 extends StatefulWidget {
  @override
  _OUT0009P02 createState() => _OUT0009P02();
}

class _OUT0009P02 extends State<OUT0009P02> {
  ...
  Future<void> _addToSearchList(Map<String, dynamic> data) async {
    ...
    if (existingIndex != -1) {
      // 기존 항목이 있으면 OUT_CONF_QTY만 업데이트
      _searchList[existingIndex]['OUT_CONF_QTY'] = data['OUT_CONF_QTY'];
      print("기존 항목의 OUT_CONF_QTY 업데이트: ${data['OUT_CONF_QTY']}");
    } else {
  ...
  Future<void> _outReleaseList() async {
    ...
    for (int i = 0; i < sendList.length; i++) {
      ...
      await transaction(context, "/out/OUT0009/insertOutItemList.do", sendParam, (status, data) async {
        if (status == Constant.resSuccessCode) {
          // 모든 항목 처리 후 마지막에 clear
  ...
  Future<dynamic> callNavi(BuildContext context, [dynamic param]) async {
    ...
    final result = await Navigator.push(
        ...
        MaterialPageRoute(
            ...
            builder: (context) => OUT0009M(param:passParam)));
            ...
            child: GestureDetector(
              ...
              child: Column(
                children: <Widget>[
                  ...
                  Expanded(
                    child: CustomGrid([ 'itemCd', 'qty'],
                      ['ITEM_NM', 'OUT_CONF_QTY'],
                      _searchList,
                      onTap: (e) {
